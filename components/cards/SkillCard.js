...
const SkillCard = ({ icon: Icon = SiPython, name, link }) => {
  const content = (
    <div
      className={`group bg-neutral-50 border-blue-700 border-2 rounded-3xl 
      p-4 md:p-6 flex flex-col items-center justify-center gap-2 md:gap-3 
      hover:bg-blue-700 hover:shadow-lg transition-all cursor-pointer aspect-square`}
    >
  ...
  // Se tiver link, torna o card clicável
  return link ? (
    <a
      href={link}
      target="_blank"
